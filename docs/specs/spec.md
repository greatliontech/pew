...
|------|------------------------------|----------|-------------------|
| `run` | serve what is proven, measure the rest, every package prepared first; one `recorded` line per measured benchmark, one `served` line per package that served anything, counting the valid recordings not re-measured (`served <pkg>: N valid, measuring M` / `N valid, nothing to run`; the row label is padded to the `recorded` column), refusals and errors as lines, progress on stderr on the cadence | `--count=10` `--benchtime=1s` `--bench=.` | `--all` (a fresh baseline after a change nothing guards), `--pin` (cut scheduler variance), `--strict` (a CI gate on hygiene), `--label` (a variant lineage), `--vouch` (discharge a version-pinned dependency variable), `--bench-dir` (a store outside the module) |
...
| `ab` | interleaved A/B of the working tree against a ref, nothing stored; the per-benchmark delta table | `--count=6` `--ref=HEAD` `--bench=.` | `--benchtime` (a budget other than go's default), `--out` (both raw streams for offline analysis), `--pin` (cut scheduler variance on both sides), `--strict` (refuse a noisy machine) |
