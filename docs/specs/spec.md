...
| key                              | meaning | source-of-truth? | class         | audit? | guard? | display                    |
|----------------------------------|---------|------------------|---------------|--------|--------|----------------------------|
| `pew-format`                     | exact Pew recording format version, currently `2` | yes | discriminator | no     | no     | format                     |
| `commit`                         | full SHA of HEAD at run time | yes | mandatory     | no     | no     | commit                     |
| `toolchain`                      | `go version` output (compiler/runtime identity) | yes | mandatory     | no     | yes    | toolchain                  |
| `machine`                        | machine fingerprint id (§8) | yes | mandatory     | no     | yes    | machine                    |
| `buildconfig`                    | digest of build tags + relevant GOFLAGS/gcflags + cgo + PGO profile **content** | yes | mandatory     | no     | yes    | buildconfig                |
| `runtimeconfig`                  | digest of Go runtime-config env (GOGC/GODEBUG/GOMEMLIMIT/GOMAXPROCS), §7 | yes | mandatory     | no     | yes    | runtimeconfig              |
| `dirty`                          | `true` if the working tree had uncommitted changes at run, the recording store (bench-dir) excluded — pew's own outputs are never part of the measured subject | yes | mandatory     | no     | no     | dirty                      |
| `pew-runconditions`              | observed transient run conditions at run time (§9) | yes | mandatory     | yes    | no     | run conditions             |
| `pew-runtime`                    | digest of runtime-input evidence (§7.8) | derived | mandatory     | no     | no     | runtime                    |
| `pew-runtime-inputs`             | encoded runtime-input manifest or incomplete disposition (§7.8) | yes | mandatory     | no     | no     | runtime inputs             |
| `pew-purity`                     | attributable Gofresh purity evidence used for this fingerprint | yes | omittable     | no     | no     | purity                     |
| `pew-vouches`                    | dynamic-state vouches that discharged culprits for this fingerprint (gofresh's sorted comma-joined identities) — audit riding the recording, never a validity key: verdicts derive from the current engine's own set, the line is omitted when no vouch was load-bearing, and two sides differing in it compare with a note, never fragmenting (the run-conditions precedent) | derived | omittable     | yes    | no     | dynamic-state vouches      |
| `pew-dynamic-state`              | the gofresh shared-dynamic-state derivation the fingerprint was computed under — a VALIDITY key like `pew-closure`: the engine refuses to serve a recording computed under another strategy, and a recording predating the key reads as the empty strategy and judges stale (`dynamic-state strategy`) — the clean-break shape, no back-fill. The comparison arm cuts per side: `stat` skips the **working-tree** side when its recorded strategy is not the current engine's (`stale (dynamic-state strategy)`, a warning and a distinct empty-comparison cause, §10) — that side is re-recordable and the one whose freshness verdict is computed, so `pew run` is the reachable remedy; a **ref-resolved** side (a pinned tag, auto's `HEAD`, either A/B ref) enters no verdict and cannot be re-run into, so it always compares and a strategy difference surfaces as a note (§10.1), never a refusal — measured numbers are strategy-independent | derived | omittable     | yes    | no     | dynamic-state strategies   |
| `pew-single-subject-discharges`  | package-level variables whose shared-dynamic-state discharge rested on the single-subject-process attestation for this fingerprint (gofresh's sorted comma-joined identities) — audit exactly as `pew-vouches`, omitted when empty | derived | omittable     | yes    | no     | single-subject discharges  |
| `pew-package-process-discharges` | package-level variables discharged by the package-process attestation's binary-scoped reachability judgment — audit exactly as `pew-vouches`, omitted when empty | derived | omittable     | yes    | no     | package-process discharges |
| `pew-closure`                    | the closure hash of the benchmark's package under the recorded derivation — the validity key the engine compares hash for hash (§7; rides in-band per the paragraph below) | derived | mandatory     | no     | no     | closure                    |
| `pew-closure-strategy`           | the gofresh closure identity derivation `pew-closure` and `pew-test-variants` were folded under (gofresh's `ClosureStrategy`) — derived and AUDIT, never a validity key: the engine judges the hash values themselves, so a derivation move re-measures a recording exactly when it moved that recording's hash, and a hash the move left equal is genuine evidence (both derivations agree on that source); `status --explain` names the move beside the hash rows on a non-valid verdict, and `stat`/`ab` note two sides recorded under different derivations. Omitted by recordings that predate the line, and kept absent by an in-place refresh, until the recording is re-measured | derived | omittable     | yes    | no     | closure derivations        |
| `pew-test-variants`              | test-variant compartment hash of the benchmark's package (§7.9) | derived | mandatory     | no     | no     | test-variants              |
| `pew-test-variant-ledger`        | encoded compartment declaration ledger — §7.9's diff base | derived | mandatory     | no     | no     | test-variant ledger        |
...
log is required, and provenance-in-band is mandatory regardless of append-vs-overwrite. A checkout
made by `git worktree add` — a linked worktree, whose objects and refs live in the main repository's
...
- **The gate never passes vacuously.** `--fail-on-regression` asserts "compared and clean", not
  ...
  the empty-comparison cause, never as an absent recording — immediately after a format change,
  "everything stale (format)" and "nothing recorded" are different diagnoses and must read
  differently. Only pew-marked recordings are inventoried (a pew-owned `pew-` key, §12 gc's same
...
a disposable detached worktree beside the repository, or in the
directory `--worktree-dir` names whenever it is given (the knob's
purpose is a parent that is unwritable or on another filesystem, but
...
refuses a ref whose build identity disagrees on any comparison guard
— a different toolchain or PGO bytes in practice, the machine and
runtime-configuration guards being one host's on both sides — with the
mismatch named before its first iteration (REQ-pew-preparation), runs
under §9's machine-hygiene regime per side (the
...
as it completes, and stops at an interruption keeping what it
persisted (REQ-pew-progress, REQ-pew-unit-persistence,
REQ-pew-interruption). `status`
and `stat` share the closure-analysis path with `run`'s default filter;
`--explain` discloses environment
...
**Verb defaults, derived.** The CLI serves a human at a terminal; `--json` is the machine-facing
output where one exists. Per verb, the default behaviour, the default output, and every default
value are derived from the verb's purpose and its reader, and every other knob is an opt-in with a
stated purpose — a knob that cannot state one is deleted; a knob several verbs share states its
purpose at least on the first row that names it, and a later row restates it only where that
verb's purpose differs. The table is the contract; the flag set and the
default values conform to it (REQ-pew-verb-defaults). The text lines the verbs print are the human
rendering and are not a stable surface — `--json`'s field names are (above); the table pins the
shape of the one text line that carries a count.
...
**REQ-pew-serve-proven** (behavior): **`run` serves what is proven and measures the rest.** `run` MUST leave a selected benchmark whose recording is valid against the current tree unmeasured by default — `--all` is the explicit re-measurement of every selected benchmark — reading one typed view of the package for both the freshness judgment and the measurement's capture: the view the judgment was made against is the view the fingerprints are captured from, never a second view built over the same subjects (the observations the engine itself performs inside a check are the engine's). *Violation:* a campaign over an unchanged tree re-measures every benchmark (count × benchtime each) to record what the store already proves, or judges freshness on one view and captures on another so a source edit between the two loads records a fingerprint the judgment never saw. *Kind:* entailed (§7's staleness contract exists so a valid recording serves; §9 spends measurement only where it is owed). *Anchor tests:* a run over a tree whose recording is valid launches no measurement and says so; a run with `--all` measures it; the run's freshness check and its capture share one view.

**REQ-pew-progress** (behavior): **No stretch stays silent.** Every verb MUST name the stretch in flight — the listing, a package's preparation with its index over the count, a package's typed load (the engine's own phases named as they begin), a build, a measurement arm with its index over the count, a comparison iteration, a store scan — on the operator's log at a fixed cadence of thirty seconds with the elapsed time, so no load, build, or measurement is silent longer than the cadence; the line is the operator's, never the machine-readable stdout's. *Violation:* a run over a large package prints nothing for the whole of its typed load, build, and every arm — minutes — and the operator cannot tell a hang from work. *Kind:* entailed (§9's own-the-run: an operator who cannot see the run cannot own it). *Anchor tests:* the reporter repeats the phase with the elapsed time on its cadence and stops with the verb; the engine's detail-free keep-alives name the analysis stretch.
...
**REQ-pew-interruption** (behavior): **An interruption keeps what was persisted and says so.** Every verb MUST bind its context to SIGINT and SIGTERM so an interruption cancels the work in flight — the process under measurement is killed, the unit whose measurement has not completed is lost — and ends the verb before its next unit with a report naming what was kept and what was not measured, exiting non-zero. A cancellation observed at any stage on the verb's path — the listing, a package's preparation, an engine construction, a guard capture, a verdict's re-observation — is reported the same way, never as that stage's own failure; a unit already measured when the interruption lands is written behind its own gate, which runs bounded and detached from the verb's context, so the gate's evidence decides it, never the signal; a cancellation that lands after the last unit completed is reported too — the verb ends by it, everything kept. *Violation:* SIGINT kills pew mid-batch: the completed arms of the package in flight are gone with no message, and the operator learns from the next status which of them re-measure. *Kind:* entailed (§9 and REQ-pew-unit-persistence: kept units are only worth keeping if the operator is told). *Anchor tests:* a run interrupted during its second arm keeps the first and reports "1 recorded, 1 not measured"; an ab interrupted after one pair reports the pair kept.
...
**REQ-pew-verb-defaults** (behavior): **The verb defaults table is the CLI's contract.** Every command and every flag the CLI serves MUST appear in §12's verb defaults table — a flag under `defaults` with the value the command holds when unset, or under `opt-ins` with its purpose stated on the first row that names it, never under both — and no row names a flag the command lacks; the guidance document's stated defaults agree with the table; `run`'s default output carries the served count for every package that served anything. *Violation:* a flag added with a purpose nobody stated, or a default changed in code and not in the table, and the surface drifts from the derivation the operator reads. *Kind:* clause-explicit (§12). *Anchor tests:* the table's verbs equal the command set and each row's flags equal the command's flag set with matching default values; a run over two recorded benchmarks after a third is added ⇒ `served <pkg>: 2 valid, measuring 1` and one `recorded` line.

**REQ-pew-admission** (behavior): **One admissibility ladder, judged over the whole recording.** Every verdict surface — `status`, `run`'s default filter, `stat`'s per-side entry — MUST climb one shared ladder over a stored recording before any guard verdict, in one order: the recording's shape and format, decoded into its fingerprint, judged over EVERY row (a recording whose rows disagree past row 0 is `stale (format)`, never a recording whose first row passes surfacing later as a mixed-provenance note); then, for a working-tree side alone, the dynamic-state strategy it was recorded under (`stale (dynamic-state strategy)`), a ref-resolved side (a pinned tag, auto's HEAD, either A/B ref) entering no verdict and comparing under any strategy with the difference surfacing as compare's audit note — so a new stale class lands in one place and cannot land on one surface only. *Violation:* a stale class added to `status` alone: `stat` compares a recording `status` calls stale, or `run` re-measures what `stat` serves. *Kind:* entailed (§7's prerequisites, §10.1's per-side skip). *Anchor tests:* rows disagreeing on the closure past row 0 ⇒ format; a working-tree side under another strategy ⇒ the strategy class, a ref side ⇒ admitted.

**REQ-pew-vouch-source** (behavior): **The reviewed vouch set has one home.** A store's standing dynamic-state vouch set MUST be read from the file `vouches` at the store's root — one `IMPORT-PATH:VARIABLE` per line, `#` comments and blank lines ignored, a malformed line refused as a malformed `--vouch` is, an absent file the empty set — by every judged verb (`run`, `status`, `stat`) for every engine over that store's module, the `--vouch` flags extending the set for one invocation and never removing from it; the store `--bench-dir` names is the store whose file governs. *Violation:* the reviewed set lives in prose and is mirrored by hand into a sweep's flag list; an extra vouch in one copy suppresses a real unverifiable verdict on that surface alone. *Kind:* entailed (§5's `pew-vouches` row records a load-bearing set; a set with two homes drifts in the unsafe direction silently). *Anchor tests:* a file naming the culprit discharges it with no flag given; file ∪ flags; a named store's file; a malformed line refuses naming the file.
