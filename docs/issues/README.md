...
| [verdict-ladder-shared-admissibility](verdict-ladder-shared-admissibility.md) | status and stat hand-order the same recording-admissibility ladder (format → dynamic-state strategy → verdict); collapse to one shared function so a new stale class cannot land on one surface only | the next chunk that extends the admissibility ladder — a new stale class or verdict surface |
