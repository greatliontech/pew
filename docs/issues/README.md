...
| [format-rung-two-readers](format-rung-two-readers.md) | the format rung is judged by store.IsRecording and by fingerprintFromConfig, two implementations of one rule | cross-tool train chunk 252 |
| [environment-normalized-once](environment-normalized-once.md) | environment validity is judged at three sites where the inputs decide it once; the two downstream guards are dead — normalize once at preparation into a typed environment | cross-tool train chunk 252 |
...
| [spec-wide-requirement-forming](spec-wide-requirement-forming.md) | only §13 is REQ-formed; a test witnessing a §§1–12 contract has no id to bind against — convert sections on demand, same structure-only discipline | cross-tool train chunk 232 (the condition fired: four §9 witnesses unbound) |
